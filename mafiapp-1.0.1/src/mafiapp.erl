...
debts(Name) ->
    F = fun() ->
        % The query list comprehension generator
        % [ResultProcessed || Result <- List, Quard]
        QH = qlc:q([if Name =:= To -> {From, 1};
                       Name =:= From -> {To, -1}
                    end ||
                    #mafiapp_services{from=From,to=To} <-
                    mnesia:table(mafiapp_services),
                    Name =:= To orelse Name =:= From]),
        % Instead of qlc:eval we use qlc:fold
        % QH is the list generated by applying the qlc generator on the table,
        % we fold the list into a dictionary
        % The dictionary is the return value of our F
    ...
    % Another list comprehension of the list generated above (here, we actually
    % execute it), we tranform the dict to a list, swap kv to sort it and sort it.
