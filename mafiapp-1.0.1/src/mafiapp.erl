...
friend_by_expertise(Expertise) ->
    % Our function evaluates a query list comprehension generator
    F = fun() ->
        qlc:eval(
            qlc:q(
                % Our list comprehension
                % [FoundProcessed || Found <- List, Guard]
                [{N,C,I,E, find_services(N)} ||
                 #mafiapp_friends{name=N,contact=C,info=I,expertise=E} <-
